...
/* GLOBAL VARIABLES */
extern int debug;
/* FIXME needs noncolilding name if it's going to be part of lib */

/* encoder.c */
//extern int jpgl_encode_mem(unsigned char **, int *, IMG_DAT *, char *);
extern int jpegl_encode_mem(unsigned char **odata, int *olen, IMG_DAT *img_dat,
		                     char *comment_text);
...
extern int putc_nistcom_jpegl(char *comment_text, const int w, const int h,
		const int d, const int ppi, const int lossyflag,
		const int n_cmpnts, int *hor_sampfctr, int *vrt_sampfctr,
		const int predict, unsigned char *odata,
		const int oalloc, int *olen);
