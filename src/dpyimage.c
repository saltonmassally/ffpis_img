...
int automatic = False;
u_int sleeptime = DEF_SLEEPTIME;
int dpy_mode = DPY_PIPE;
int raw = False;
u_int raw_w, raw_h, raw_depth, raw_whitepix;
char def_tmpdir[] = DEF_TMPDIR;
char *tmpdir = def_tmpdir;

/************************************************************************/
int dpyimage(char *fname, register u_char *data, u_int image_w, u_int image_h,
             u_int d, u_int whitepix, int align, int *done)
{
   ...
   u_char *windata;
   char *data24;
   u_int max_whitepix = ((1 << BITSPERBYTE) - 1);
   u_int windatasize;
   u_int new_ww, new_wh;
   ...
   if (first) {
      ...
      if ((depth != 1) &&
         ...
         (void) fprintf(stderr,
                        ...
                        program,filename,depth,BITSPERBYTE);
         return(-2);
      }
   ...
   if (depth == 1)
      windatasize = howmany(ww,BITSPERBYTE) * wh;
   else if (depth == 8)
      windatasize = ww * wh;
   else /* if (depth == 8) */
      windatasize = ww * wh * BITMAP_UNIT_24;

   windata = (u_char *) malloc(windatasize);
   if (windata == (u_char *) NULL) {
   ...
   if (depth == 1) {
      ...
      if (visual->class == TrueColor) {
         XMGetSubImageDataDepth(((char *)data),relx,rely,iw,ih,
                                ((char *)windata),ww,wh);
         if(ret = ImageBit8ToBit24Unit32(&data24, windata, ww, wh)){
            free(windata);
            return(ret);
         }
         free(windata);
...
/*******************************************************************/
int event_handler(register XImage *image, register u_char *data, int *done)
{
   int ret;
   XEvent event;
   int quit;
   ...
   if (automatic) {
      if (! window_up) {
         window_up = True;
         XMaskEvent(display,(u_long)ExposureMask,&event);
         refresh_window(image);
      }
      sleep(sleeptime);
      return;
...
/************************************************************/
int drag_image(register XImage *image, register u_char *data, int dx, int dy)
{
   int ret;
   int px, py;
   ...
   dx = (dx / BITSPERBYTE) * BITSPERBYTE;
   dy = (dy / BITSPERBYTE) * BITSPERBYTE;
...
/************************************************************/
int move_image(register XImage *image, register u_char *data, int px, int py)
{
   int ret;
   char *data8, *data24;
...
/************************************************************/
int button_release(XEvent *event, register XImage *image, u_char *data)
{
   int ret;
   int x, y, on;
   u_int state;
