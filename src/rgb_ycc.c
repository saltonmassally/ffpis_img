                ...
                const int intrlvflag)
{
   ...
   if(intrlvflag){
      ret = rgb2ycc_intrlv_mem(odata, olen, idata, width, height, depth);
      if(ret)
         return(ret);
   }
   else{
                      ...
                      unsigned char *idata, const int iw, const int ih)
{
   ...
   ow = ceil(iw / (double)win_w);
   oh = ceil(ih / (double)win_h);
   last_win_w = iw % win_w;
   if(last_win_w == 0)
      last_win_w = win_w;
   last_win_h = ih % win_h;
                ...
                const int intrlvflag)
{
   ...
   if(intrlvflag){
      ret = ycc2rgb_intrlv_mem(odata, olen, idata, width, height, depth);
      if(ret)
         return(ret);
   }
   else{
               ...
               int *hor_sampfctr, int *vrt_sampfctr, const int n_cmpnts)
{
   ...
   for(i = 0; i < n_cmpnts; i++){
      ...
      samp_width[i] = ceil(width *
                                (hor_sampfctr[i] / (double)max_hor));
      samp_height[i] = ceil(height *
