...
int read_jfif_header(JFIF_HEADER **ojfif_header, FILE *infp)
{
   ...
   /* table_len */
   ret = read_ushort(&table_len, infp);
   if(ret){
      free(jfif_header);
      return(-3);
   }
   ...
   for(i = 0; i < JFIF_IDENT_LEN; i++){
      ret = read_byte(&(jfif_header->ident[i]), infp);
      if(ret){
         free(jfif_header);
         return(-4);
      }
   ...
   ret = read_ushort(&(jfif_header->ver), infp);
   if(ret){
      free(jfif_header);
      return(-6);
   }
   ret = read_byte(&(jfif_header->units), infp);
   if(ret){
      free(jfif_header);
      return(-7);
   }
   ret = read_ushort(&(jfif_header->dx), infp);
   if(ret){
      free(jfif_header);
      return(-8);
   }
   ret = read_ushort(&(jfif_header->dy), infp);
   if(ret){
      free(jfif_header);
      return(-9);
   }
   ret = read_byte(&(jfif_header->tx), infp);
   if(ret){
      free(jfif_header);
      return(-10);
   }
   ret = read_byte(&(jfif_header->ty), infp);
   if(ret){
      free(jfif_header);
      return(-11);
   }
                     ...
                     unsigned char **cbufptr, unsigned char *ebufptr)
{
   ...
   /* Ident */
   for(i = 0; i < JFIF_IDENT_LEN; i++){
      ret = getc_byte(&(jfif_header->ident[i]), cbufptr, ebufptr);
      if(ret){
         free(jfif_header);
         return(-4);
      }
   ...
   ret = getc_ushort(&(jfif_header->ver), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-6);
   }
   ret = getc_byte(&(jfif_header->units), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-7);
   }
   ret = getc_ushort(&(jfif_header->dx), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-8);
   }
   ret = getc_ushort(&(jfif_header->dy), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-9);
   }
   ret = getc_byte(&(jfif_header->tx), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-10);
   }
   ret = getc_byte(&(jfif_header->ty), cbufptr, ebufptr);
   if(ret){
      free(jfif_header);
      return(-11);
   }
...
int write_jfif_header(JFIF_HEADER *jfif_header, FILE *outfp)
{
   ...
   /* APP0 0xffe0 */
   ret = write_ushort(APP0, outfp);
   if(ret)
      return(ret);

   /* Table_len */
   ret = write_ushort(table_len, outfp);
   if(ret)
      return(ret);

   /* Ident */
   for(i = 0; i < JFIF_IDENT_LEN; i++) {
      ret = write_byte(jfif_header->ident[i], outfp);
      if(ret)
         return(ret);
   }

   ret = write_ushort(jfif_header->ver, outfp);
   if(ret)
      return(ret);
   ret = write_byte(jfif_header->units, outfp);
   if(ret)
      return(ret);
   ret = write_ushort(jfif_header->dx, outfp);
   if(ret)
      return(ret);
   ret = write_ushort(jfif_header->dy, outfp);
   if(ret)
      return(ret);
   ret = write_byte(jfif_header->tx, outfp);
   if(ret)
      return(ret);
   ret = write_byte(jfif_header->ty, outfp);
   if(ret)
      return(ret);
                     ...
                     const int outalloc, int *outlen)
{
   ...
   /* APP0 0xffe0 */
   ret = putc_ushort(APP0, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ...
   /* Ident */
   for(i = 0; i < JFIF_IDENT_LEN; i++) {
      ret = putc_byte(jfif_header->ident[i], outbuf, outalloc, outlen);
      if(ret)
         return(ret);
   }

   ret = putc_ushort(jfif_header->ver, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ret = putc_byte(jfif_header->units, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ret = putc_ushort(jfif_header->dx, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ret = putc_ushort(jfif_header->dy, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ret = putc_byte(jfif_header->tx, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ret = putc_byte(jfif_header->ty, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
                     ...
                     FILE *infp)
{
   ...
   switch(marker){
   case DHT:
      ret = read_huffman_table_jpegl(huf_table, infp);
      if(ret)
         return(ret);
      break;
   case COM:
      ret = read_comment(&comment, infp);
      if(ret)
         return(ret);
#ifdef PRINT_COMMENT
                     ...
                     unsigned char **cbufptr, unsigned char *ebufptr)
{
   ...
   switch(marker){
   case DHT:
      ret = getc_huffman_table_jpegl(huf_table, cbufptr, ebufptr);
      if(ret)
         return(ret);
      break;
   case COM:
...
int read_frame_header_jpegl(FRM_HEADER_JPEGL **ofrm_header, FILE *infp)
{
   ...
   /* Lf */
   ret = read_ushort(&Lf, infp);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* P */
   ret = read_byte(&(frm_header->prec), infp);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* Y */
   ret = read_ushort(&(frm_header->y), infp);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* X */
   ret = read_ushort(&(frm_header->x), infp);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* Nf */
   ret = read_byte(&(frm_header->Nf), infp);
   if(ret){
      free(frm_header);
      return(ret);
   }
   ...
   for(i = 0; i < frm_header->Nf; i++) {
      /* C */
      ret = read_byte(&(frm_header->C[i]), infp);
      if(ret){
         free(frm_header);
         return(ret);
      }
      /* HV */
      ret = read_byte(&(frm_header->HV[i]), infp);
      if(ret){
         free(frm_header);
         return(ret);
      }
      /* Tq */
      ret = read_byte(&(frm_header->Tq[i]), infp);
      if(ret){
         free(frm_header);
         return(ret);
      }
                      ...
                      unsigned char **cbufptr, unsigned char *ebufptr)
{
   ...
   /* Lf */
   ret = getc_ushort(&Lf, cbufptr, ebufptr);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* P */
   ret = getc_byte(&(frm_header->prec), cbufptr, ebufptr);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* Y */
   ret = getc_ushort(&(frm_header->y), cbufptr, ebufptr);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* X */
   ret = getc_ushort(&(frm_header->x), cbufptr, ebufptr);
   if(ret){
      free(frm_header);
      return(ret);
   }
   /* Nf */
   ret = getc_byte(&(frm_header->Nf), cbufptr, ebufptr);
   if(ret){
      free(frm_header);
      return(ret);
   }
   ...
   for(i = 0; i < frm_header->Nf; i++) {
      /* C */
      ret = getc_byte(&(frm_header->C[i]), cbufptr, ebufptr);
      if(ret){
         free(frm_header);
         return(ret);
      }
      /* HV */
      ret = getc_byte(&(frm_header->HV[i]), cbufptr, ebufptr);
      if(ret){
         free(frm_header);
         return(ret);
      }
      /* Tq */
      ret = getc_byte(&(frm_header->Tq[i]), cbufptr, ebufptr);
      if(ret){
         free(frm_header);
         return(ret);
      }
...
int write_frame_header_jpegl(FRM_HEADER_JPEGL *frm_header, FILE *outfp)
{
   ...
   /* SOF3 */
   ret = write_ushort(SOF3, outfp);
   if(ret)
      return(ret);
   /* Lf */
   ret = write_ushort((8+(3*frm_header->Nf)), outfp);
   if(ret)
      return(ret);
   /* P */
   ret = write_byte(frm_header->prec, outfp);
   if(ret)
      return(ret);
   /* Y */
   ret = write_ushort(frm_header->y, outfp);
   if(ret)
      return(ret);
   /* X */
   ret = write_ushort(frm_header->x, outfp);
   if(ret)
      return(ret);
   /* Nf */
   ret = write_byte(frm_header->Nf, outfp);
   if(ret)
      return(ret);

   for(i = 0; i < frm_header->Nf; i++) {
      /* C */
      ret = write_byte(frm_header->C[i], outfp);
      if(ret)
         return(ret);
      /* HV */
      ret = write_byte(frm_header->HV[i], outfp);
      if(ret)
         return(ret);
      /* Tq */
      ret = write_byte(frm_header->Tq[i], outfp);
      if(ret)
         return(ret);
   }
                       ...
                       const int outalloc, int *outlen)
{
   ...
   /* SOF3 */
   ret = putc_ushort(SOF3, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Lf */
   ret = putc_ushort((8+(3*frm_header->Nf)), outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* P */
   ret = putc_byte(frm_header->prec, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Y */
   ret = putc_ushort(frm_header->y, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* X */
   ret = putc_ushort(frm_header->x, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Nf */
   ret = putc_byte(frm_header->Nf, outbuf, outalloc, outlen);
   if(ret)
      return(ret);

   for(i = 0; i < frm_header->Nf; i++) {
      /* C */
      ret = putc_byte(frm_header->C[i], outbuf, outalloc, outlen);
      if(ret)
         return(ret);
      /* HV */
      ret = putc_byte(frm_header->HV[i], outbuf, outalloc, outlen);
      if(ret)
         return(ret);
      /* Tq */
      ret = putc_byte(frm_header->Tq[i], outbuf, outalloc, outlen);
      if(ret)
         return(ret);
   }
...
int read_scan_header(SCN_HEADER **oscn_header, FILE *infp)
{
   ...
   /* Ls */
   ret = read_ushort(&Ls, infp);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Ns */
   ret = read_byte(&(scn_header->Ns), infp);
   if(ret){
      free(scn_header);
      return(ret);
   }
   ...
   for(i = 0; i < scn_header->Ns; i++) {
      /* Cs */
      ret = read_byte(&(scn_header->Cs[i]), infp);
      if(ret){
         free(scn_header);
         return(ret);
      }
      /* Tda */
      ret = read_byte(&(scn_header->Tda[i]), infp);
      if(ret){
         free(scn_header);
         return(ret);
      }
   ...
   /* Ss */
   ret = read_byte(&(scn_header->Ss), infp);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Se */
   ret = read_byte(&(scn_header->Se), infp);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Ahl */
   ret = read_byte(&(scn_header->Ahl), infp);
   if(ret){
      free(scn_header);
      return(ret);
   }
         ...
         unsigned char *ebufptr)
{
   ...
   /* Ls */
   ret = getc_ushort(&Ls, cbufptr, ebufptr);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Ns */
   ret = getc_byte(&(scn_header->Ns), cbufptr, ebufptr);
   if(ret){
      free(scn_header);
      return(ret);
   }
   ...
   for(i = 0; i < scn_header->Ns; i++) {
      /* Cs */
      ret = getc_byte(&(scn_header->Cs[i]), cbufptr, ebufptr);
      if(ret){
         free(scn_header);
         return(ret);
      }
      /* Tda */
      ret = getc_byte(&(scn_header->Tda[i]), cbufptr, ebufptr);
      if(ret){
         free(scn_header);
         return(ret);
      }
   ...
   /* Ss */
   ret = getc_byte(&(scn_header->Ss), cbufptr, ebufptr);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Se */
   ret = getc_byte(&(scn_header->Se), cbufptr, ebufptr);
   if(ret){
      free(scn_header);
      return(ret);
   }
   /* Ahl */
   ret = getc_byte(&(scn_header->Ahl), cbufptr, ebufptr);
   if(ret){
      free(scn_header);
      return(ret);
   }
...
int write_scan_header(SCN_HEADER *scn_header, FILE *outfp)
{
   ...
   /* SOS */
   ret = write_ushort(SOS, outfp);
   if(ret)
      return(ret);
   /* Ls */
   ret = write_ushort((6+(2*scn_header->Ns)), outfp);
   if(ret)
      return(ret);
   /* Ns */
   ret = write_byte(scn_header->Ns, outfp);
   if(ret)
      return(ret);

   for(i = 0; i < scn_header->Ns; i++) {
      /* Cs */
      ret = write_byte(scn_header->Cs[i], outfp);
      if(ret)
         return(ret);
      /* Tda */
      ret = write_byte(scn_header->Tda[i], outfp);
      if(ret)
         return(ret);
   }

   /* Ss */
   ret = write_byte(scn_header->Ss, outfp);
   if(ret)
      return(ret);
   /* Se */
   ret = write_byte(scn_header->Se, outfp);
   if(ret)
      return(ret);
   /* Ahl */
   ret = write_byte(scn_header->Ahl, outfp);
   if(ret)
      return(ret);
                     ...
                     const int outalloc, int *outlen)
{
   ...
   /* SOS */
   ret = putc_ushort(SOS, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Ls */
   ret = putc_ushort((6+(2*scn_header->Ns)), outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Ns */
   ret = putc_byte(scn_header->Ns, outbuf, outalloc, outlen);
   if(ret)
      return(ret);

   for(i = 0; i < scn_header->Ns; i++) {
      /* Cs */
      ret = putc_byte(scn_header->Cs[i], outbuf, outalloc, outlen); 
      if(ret)
         return(ret);
      /* Tda */
      ret = putc_byte(scn_header->Tda[i], outbuf, outalloc, outlen);
      if(ret)
         return(ret);
   }

   /* Ss */
   ret = putc_byte(scn_header->Ss, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Se */
   ret = putc_byte(scn_header->Se, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   /* Ahl */
   ret = putc_byte(scn_header->Ahl, outbuf, outalloc, outlen);
   if(ret)
      return(ret);
   ...
   FILE *infp)            /* input file */
{
   ...
   ret = fread(comment, sizeof(unsigned char), cs, infp);
   if(ret != cs){
      fprintf(stderr,
   ...
   FILE *outfp)            /* output file */
{
   ...
   ret = write_ushort(marker, outfp);
   if(ret)
      return(ret);
   /* comment size */
   hdr_size = 2 + cs;
   ret = write_ushort(hdr_size, outfp);
   if(ret)
      return(ret);

   ret = fwrite(comment, cs, sizeof(unsigned char), outfp);
   if(cs != ret){
     fprintf(stderr,
                      ...
                      const int ilen, unsigned char *comment)
{
   ...
   if(marker == APP0){
      ...
      /* Copy JFIF header. */
      ret = putc_jfif_header(jfif_header, ndata, nalloc, &nlen);
      if(ret){
         free(ndata);
         free(jfif_header);
         return(ret);
   ...
   if (marker == COM){
      ...
      do{
         ...
         /* Copy COM segment. */
	 ret = putc_comment(COM, ocomment,
			 strlen(ocomment), ndata, nalloc, &nlen);
         if(ret){
            free(ndata);
            free(ocomment);
            return(ret);
   ...
   ret = putc_comment(COM, comment, strlen(comment), ndata, nalloc, &nlen);
   if(ret){
      free(ndata);
      return(ret);
   }
                       ...
                       unsigned char *odata, const int oalloc, int *olen)
{
   ...
   ret = combine_jpegl_nistcom(&nistcom, w, h, d, ppi, lossyflag,
   ...
   ret = putc_comment(COM, comstr, strlen(comstr), odata, oalloc, olen);
   if(ret){
      freefet(nistcom);
      free(comstr);
      return(ret);
   ...
   if(gencomflag){
      ...
      ret = putc_comment(COM, comment_text, strlen(comment_text),
		      odata, oalloc, olen);
      if(ret)
         return(ret);
   }
