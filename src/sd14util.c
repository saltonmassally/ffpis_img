                      ...
                      int *odepth, int *lossyflag, FILE *infp)
{
   ...
   ret = read_marker_wsq(&marker, SOI_WSQ, infp);
   if(ret){
      return(ret);
   }

   /* Read in supporting tables up to the SOF_WSQ marker. */
   ret = read_marker_wsq(&marker, TBLS_N_SOF, infp);
   if(ret){
      return(ret);
   }
   while(marker != SOF_WSQ) {
      ret = read_table_wsq14(marker, &dtt_table, &dqt_table, dht_table, infp);
      if(ret){
         return(ret);
      }
      ret = read_marker_wsq(&marker, TBLS_N_SOF, infp);
   ...
   ret = read_frame_header_wsq(&frm_header_wsq, infp);
   if(ret){
      return(ret);
   }
   ...
   ret = huffman_decode_data_file_wsq14(qdata, &dtt_table, &dqt_table,
		   dht_table, infp);
   if(ret){
      free(qdata);
      return(ret);
   ...
   ret = unquantize(&fdata, &dqt_table, q_tree,
		   Q_TREELEN, qdata, width, height);
   if(ret){
      free(qdata);
      return(ret);
   }
...
/**********************************/
   ...
   /* Decode the Huffman encoded data blocks. */
   ret = huffman_decode_data_file_wsq14(qdata, &dtt_table,
		   &dqt_table, dht_table, infp);
   if(ret){
      free(qdata);
      return(ret);
   }
...
/*************************************/
   ...
   ret = unshuffle_wsq14(&fdata, &dqt_table, q_tree,
		   Q_TREELEN, qdata, width, height);
   if(ret){
      free(qdata);
      return(ret);
   ...
   ret = shuffle_dqt_wsq14(&dqt_table);
   if(ret)
      return(ret);

   ret = shuffle_wsq14(&qdata, &qsize, dqt_table, q_tree_wsq14,
		   Q_TREELEN, fdata, width, height);
   if(ret){
      free(fdata);
      return(ret);
   }
...
/***********************************/
   ...
   ret = putc_frame_header_wsq(width, height, frm_header_wsq.m_shift,
		   frm_header_wsq.r_scale, wsq_data, wsq_alloc, &wsq_len);
   if(ret){
      free(qdata);
      free(wsq_data);
      return(ret);
   ...
   ret = putc_huffman_table(DHT_WSQ, 0, huffbits, huffvalues, wsq_data,
		   wsq_alloc, &wsq_len);
   if(ret){
      free(qdata);
      free(wsq_data);
   ...
   ret = putc_huffman_table(DHT_WSQ, 1, huffbits, huffvalues,
   ...
   ret = compress_block(huff_buf, &hsize3, qdata+qsize1+qsize2,
		   qsize3, MAX_HUFFCOEFF, MAX_HUFFZRUN, hufftable);
   if(ret){
      free(qdata);
      free(wsq_data);
      free(huff_buf);
   ...
   FILE *infp)            /* input file */
{
   ...
   switch(marker){
   case DTT_WSQ:
      ret = read_transform_table(dtt_table, infp);
      if(ret)
         return(ret);
      break;
   case DQT_WSQ:
      ret = read_quantization_table(dqt_table, infp);
      if(ret)
         return(ret);
      break;
   case DHT_WSQ:
      ret = read_huff_table_wsq14(dht_table, infp);
      if(ret)
         return(ret);
      break;
   case COM_WSQ:
                     ...
                     const int width, const int height)
{
   ...
   (void)w_treelen,(void)q_treelen,(void)q_treelen_wsq14;
   /* FIXME UNUSED */
   ...
   FILE *infp)            /* input file */
{
   ...
   ret = read_marker_wsq(&marker, TBLS_N_SOB, infp);
   if(ret)
      return(ret);
   ...
   while(marker != EOI_WSQ) {
      ...
      if(marker != 0) {
         ...
         while(marker != SOB_WSQ) {
            ret = read_table_wsq14(marker, dtt_table,
			    dqt_table, dht_table, infp);
            if(ret)
               return(ret);
	    ret = read_marker_wsq(&marker, TBLS_N_SOB, infp);
	 ...
	 ret = read_block_header(&hufftable_id, infp);
         if(ret)
            return(ret);
	 ...
	 ret = build_huffsizes(&hufftable, &last_size,
      ...
      ret = decode_data_file(&nodeptr, mincode, maxcode, valptr,
		      (dht_table+hufftable_id)->huffvalues, infp,
		      &bit_count, &marker);
      if(ret)
         return(ret);
      ...
      if(nodeptr <= 100)
         ...
         *ip++ = nodeptr - 180;
         ...
         ret = nextbits_wsq(&tbits, &marker, infp, &bit_count, 8);
         if(ret)
            return(ret);
         *ip++ = tbits;
      }
         ...
         *ip++ = -tbits;
         ...
         ret = nextbits_wsq(&tbits, &marker, infp, &bit_count, 16);
         if(ret)
            return(ret);
         *ip++ = tbits;
      }
