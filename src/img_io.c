...
#cat: read_raw_or_ihead - reads a pixmap from either an IHead or
...
/************************************************************/
int read_raw_or_ihead(const int iheadflag, char *ifile, IHEAD **ohead,
                unsigned char **odata, int *owidth, int *oheight, int *odepth)
{
   ...
   if(iheadflag) {
      ...
      if((depth != 8) && (depth != 24)){
         free(ihead);
         free(idata);
         fprintf(stderr, "ERROR: read_raw_or_ihead : ");
   ...
   /* Otherwise, input image is a raw pixmap... */
      ...
      if(idata == (unsigned char *)NULL) {
         fprintf(stderr,"ERROR : read_raw_or_ihead : malloc : idata\n");
         return(-4);
      }
      /* Open the input image file for reading ... */
      if((infp = fopen(ifile, "rb")) == (FILE *)NULL) {
         fprintf(stderr, "ERROR: read_raw_or_ihead : %s\n", ifile);
         return(-5);
      }
      ...
      if(img_siz != num_pix) {
         free(idata);
         fprintf(stderr, "ERROR : read_raw_or_ihead : fread : ");
               ...
               unsigned char **odata, int *owidth, int *oheight, int *odepth)
{
   ...
   ret = read_raw_or_ihead(iheadflag, ifile, ohead, odata,
   ...
   if(width < MIN_IMG_DIM || height < MIN_IMG_DIM) {
      ...
      free(*odata);
      fprintf(stderr,
              "ERROR: read_raw_or_ihead : Image must be at least %d X %d\n",
