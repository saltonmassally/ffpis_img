   ...
   FILE *infp)            /* input file */
{
   ...
   case DQT_WSQ:
      ...
      break;
   case DHT_WSQ:
      ret = read_huffman_table_wsq(dht_table, infp);
      if(ret)
         return(ret);
      break;
   case COM_WSQ:
   ...
   FILE *infp)            /* input file */
{
   ...
   if(ret)
      return(ret);
   ret = read_byte(&(dtt_table->hisz), infp);
   if(ret)
      return(ret);
   ret = read_byte(&(dtt_table->losz), infp);
   if(ret)
      return(ret);
   ...
   FILE *outfp)      /* compressed file */
{
   ...
   ret = write_ushort(DTT_WSQ, outfp);
   if(ret)
      return(ret);
   ret = write_ushort(58, outfp);
   if(ret) /* table size */
      return(ret);
   /* number analysis lowpass coefficients */
   ret = write_byte(losz, outfp);
   if(ret)
      return(ret);
   /* number analysis highpass coefficients */
   ret = write_byte(hisz, outfp);
   if(ret)
      return(ret);
   ...
   for(coef = (losz>>1); coef < (unsigned long int)losz; coef++) {
      ...
      ret = write_byte(sign, outfp);
      if(ret)
         return(ret);
      ret = write_byte(scale_ex, outfp);
      if(ret)
         return(ret);
      ret = write_uint(int_dat, outfp);
      if(ret)
         return(ret);
   }
   ...
   FILE *outfp)               /* compressed file */
{
   ...
   ret = write_ushort(DQT_WSQ, outfp);
   if(ret)
      return(ret);
   /* table size */
   ret = write_ushort(389, outfp);
   if(ret)
      return(ret);
   /* exponent scaling value */
   ret = write_byte(2, outfp);
   if(ret)
      return(ret);
   /* quantizer bin center parameter */
   ret = write_ushort(44, outfp);
   if(ret)
      return(ret);
   ...
   for(sub = 0; sub < 64; sub ++) {
      ...
      if(ret)
         return(ret);
      ret = write_ushort(shrt_dat, outfp);
      if(ret)
         return(ret);
      ret = write_byte(scale_ex2, outfp);
      if(ret)
         return(ret);
      ret = write_ushort(shrt_dat2, outfp);
      if(ret)
         return(ret);
   }
   ...
   FILE *infp)            /* input file */
{
   ...
   ret = read_huffman_table(&table_id, &huffbits, &huffvalues,
		   MAX_HUFFCOUNTS_WSQ, infp, READ_TABLE_LEN, &bytes_left);
   if(ret)
      return(ret);
   ...
   FILE *infp)              /* input file */
{
   ...
   if(ret)
      return(ret);
   ret = read_byte(&(frm_header->black), infp);
   if(ret)
      return(ret);
   ret = read_byte(&(frm_header->white), infp);
   if(ret)
      return(ret);
   ret = read_ushort(&(frm_header->height), infp);
   if(ret)
      return(ret);
   ret = read_ushort(&(frm_header->width), infp);
   if(ret)
      return(ret);
   ret = read_byte(&scale, infp);
   if(ret)
   ...
   ret = read_byte(&(frm_header->wsq_encoder), infp);
   if(ret)
      return(ret);
   ret = read_ushort(&(frm_header->software), infp);
   if(ret)
      return(ret);
   ...
   FILE *outfp)           /* compressed file          */
{
   ...
   ret = write_ushort(SOF_WSQ, outfp);
   if(ret)
      return(ret);
   /* size of frame header */
   ret = write_ushort(17, outfp);
   if(ret)
      return(ret);
   /* black pixel */
   ...
   if(ret)
      return(ret);
   /* white pixel */
   ret = write_byte(255, outfp);
   if(ret)
      return(ret);
   ret = write_ushort(height, outfp);
   if(ret)
      return(ret);
   ret = write_ushort(width, outfp);
   if(ret)
      return(ret);
   ...
   FILE *outfp)       /* compressed file */
{
   ...
   ret = write_ushort(SOB_WSQ, outfp);
   if(ret)
      return(ret);
   /* block header size */
   ret = write_ushort(3, outfp);
   if(ret)
      return(ret);
   ret = write_byte(table, outfp);
   if(ret)
      return(ret);
...
int read_nistcom_wsq(NISTCOM **onistcom, FILE *infp)
{
   ...
   /* Get next marker. */
   ret = read_marker_wsq(&marker, ANY_WSQ, infp);
   if(ret)
      return(ret);
