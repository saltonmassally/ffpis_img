                   ...
                   int *hor_sampfctr, int *vrt_sampfctr, const int n_cmpnts)
{
   ...
   for(c = 0; c < n_cmpnts; c++){
      ...
      samp_width[c] = ceil(width * (hor_sampfctr[c] / (double)max_hor));
      samp_height[c] = ceil(height * (vrt_sampfctr[c] / (double)max_vrt));
                   ...
                   int *hor_sampfctr, int *vrt_sampfctr, const int n_cmpnts)
{
   ...
   ret = pad_component_planes(idata, &olen, new_samp_width, new_samp_height,
		   samp_width, samp_height, pad_width, pad_height, n_cmpnts);
   if(ret)
      return(ret);
                       ...
                       const int n_cmpnts)
{
   ...
   for(c = 0; c < n_cmpnts; c++){
      ...
      samp_width[c] = ceil(width * (hor_sampfctr[c] / (double)max_hor));
      pad_width[c] = (ceil(samp_width[c]/(double)hor_sampfctr[c]) *
                     hor_sampfctr[c]) - samp_width[c];

      /* Compute the pixel height of the component's output plane. */
      samp_height[c] = ceil(height * (vrt_sampfctr[c] / (double)max_vrt));
      pad_height[c] = (ceil(samp_height[c]/(double)vrt_sampfctr[c]) *
                   ...
                   int *vrt_sampfctr, const int n_cmpnts, const int intrlvflag)
{
   ...
   for(c = 0; c < n_cmpnts; c++){
      ...
      samp_width[c] = ceil(w * (hor_sampfctr[c] / (double)max_hor));
      samp_height[c] = ceil(h * (vrt_sampfctr[c] / (double)max_vrt));
      pad_width[c] = 0;
      pad_height[c] = 0;
   }
