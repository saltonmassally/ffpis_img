                       ...
                       FILE *infp, const int read_table_len, int *bytes_left)
{
   ...
   for(i = 0; i < num_hufvals; i++){
      ret = read_byte(&(huffvalues[i]), infp);
      if(ret){
         free(huffbits);
         free(huffvalues);
         return(ret);
   ...
   FILE  *outfp)         /* output file              */
{
   ...
   /* Table ID */
   ret = write_byte(table_id, outfp);
   if(ret)
      return(ret);

   /* Huffbits (MAX_HUFFBITS) */
   for(i = 0; i < MAX_HUFFBITS; i++){
      ret = write_byte(huffbits[i], outfp);
      if(ret)
         return(ret);
   }
   ...
   for(i = 0; i < table_len-values_offset; i++){
      ret = write_byte(huffvalues[i], outfp); 
      if(ret)
         return(ret);
   }
